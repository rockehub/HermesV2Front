<template>
  <Popper class="flex" placement="bottom-end" offsetDistance="12">
    ...
    >
      ...
      <span v-if="notification.getNotifications.length > 0" class="absolute -top-px -right-px flex h-3 w-3 items-center justify-center">
                    <span
      ...
      >
        ...
        >
          ...
          <div class="is-scrollbar-hidden flex shrink-0 overflow-x-auto px-3">
            <button v-for="(tab, index) in getAllNotificationsTypes()" :key="index"
                    @click="activeTab = tab"
                    :class="activeTab === tab ? 'border-primary dark:border-accent text-primary dark:text-accent-light' : 'border-transparent hover:text-slate-800 focus:text-slate-800 dark:hover:text-navy-100 dark:focus:text-navy-100'"
        ...
        <div class="tab-content flex flex-col overflow-hidden">
          <div v-for="(tab, index) in getAllNotificationsTypes()" :key="index"
               v-show="activeTab === tab"
               class="is-scrollbar-hidden space-y-4 overflow-y-auto px-4 py-4"
          >
            <div class="flex items-center space-x-3" v-for="(notification,index) in filterNotificationsByType(tab)"
                 :key="index">
              <div
...
export default defineComponent({
  ...
  methods: {
    getAllNotificationsTypes() {
      let types: Array<string> = this.notification.getNotifications.map(notification => notification.type)
      //add prefix message.notification to all types
      types = types.map(type => `message.notification.${type}`)
      // remove duplicates
      return [...new Set(types)]
    },
    filterNotificationsByType(type: string) {
      //remove message.notification
      type = type.replace('message.notification.', '')
      return this.notification.getNotifications.filter(notification => notification.type === type)
    },
    firstType() {
      return this.getAllNotificationsTypes()[0]
    },
  },
  mounted() {
    this.activeTab = this.firstType()
  }
})
</script>